/* Read the RAPL registers on a sandybridge-ep machine or newer       */
...
/*   Taylor Kidd -- taylor.w.kidd @ intel.com modified for pwr expts  */
...
*/
  ...
  //  time_t current_time = time(NULL);
  if(current_time != -1)
    printf(">>>time: %ju sec Epoch / %s",
	   ((uintmax_t)current_time), asctime(gmtime(&current_time)));
  ...
  */
  ...
  /*  if(current_time != -1)
    printf("The current time is %s(%ju seconds since the Epoch)\n",
	   asctime(gmtime(&current_time)), (uintmax_t)current_time);
  */
  /*
  printf("\n");
