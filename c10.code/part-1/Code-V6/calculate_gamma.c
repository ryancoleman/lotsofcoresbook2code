...
_OFFLOADABLE void calculate_gamma_3d(thread_g3d_data_t* thread_data, int maxblocksize)
{
    ...
    double (*restrict mmat)[terms] = (double (*restrict)[terms]) thread_data[tid].mmat;
    double* xdiff = thread_data[tid].xdiff;
    double* ixdiff = thread_data[tid].ixdiff;
    double xmax = xvec[xsize-1];
    double (*restrict x)[terms] = (double (*restrict)[terms]) thread_data[tid].intgrlvec;
    double (*restrict plijkz)[terms] = (double (*restrict )[terms]) thread_data[tid].plijkz;
    ...
    while (hasWork)
    {
        ...
        for (i=start_i; i<end_i; i++)
        {
            ...
            for (j=start_j; j<end_j; j++)
            {
                ...
                for (k = start_k; k < end_k; k += 2) 
                {
                    ...
                    { 
                        x[l][n] = calculate_xint(i,j,k,n,xsize,xvec,yvec,thread_data[tid].task_spl);
                    }
                    l++;
                }
            }
        ...
        // store product of x[][] and plijkz[][] in mmat[][]
        for (m=start_m; m<end_m; m+=nworkers)
        {
            #pragma omp simd private(n,l)
            for (n=0; n<terms; n++)
            {
                double tmp = mmat[m][n];
                for (l=0; l<blocksize; l++)
                {
                    tmp += x[l][n] * plijkz[l][m];
                }
                mmat[m][n] = tmp;
            }
        }
