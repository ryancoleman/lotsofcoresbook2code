...
_OFFLOADABLE void calculate_gamma_3d(thread_g3d_data_t* thread_data)
{
    ...
    double (*mmat)[terms] = (double (*)[terms])  thread_data[tid].mmat;
    ...
    int i1 = thread_data[tid].i1;
    int j1 = thread_data[tid].j1;
    int k1 = thread_data[tid].k1;
    ...
    for (i=start_i; i<end_i; i++)
    {
        ...
        for (j=start_j; j<end_j; j++)
        {
            ...
            for (k = start_k; k < end_k; k += 2) 
            {
                ...
                int end_n = terms;
                for (int n = start_n; n < end_n; n+=nworkers)
                { 
                    x[n] = calculate_xint(i,j,k,n,xsize,xvec,yvec,thread_data[tid].task_spl);
                }
                ...
                for (int m = start_m; m < end_m; m+=nworkers)
                {
                    int p1,p2,p3;
