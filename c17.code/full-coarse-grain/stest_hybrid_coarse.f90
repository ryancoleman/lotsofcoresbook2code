...
!  The 7-point filter around the centre-point in a 3-d lattice or
!  "stencil" structure arises quite often numerically, and poses a 
!  performance challenge for scalar processors!
!
!  The program reads 8 input variables, from 6 lines of standard input,
!  (11 variables over 7 lines for MPI) 
...
!    "Thread-Local" indices have subscript _tloc
!
...
!
     ...
     &         k_glob, i_loc, j_loc, k_loc, i,j,k
       integer iseed, icount, maxcount
       integer num_threads,mythread,nz_tloc,ny_tloc
       ...
       real    rparam(2)
       include 'mpif.h'
       integer istatus(MPI_STATUS_SIZE), irequired, iprovided
!
...
!      call MPI_Init(ierror)
       ...
       call MPI_Init_Thread(irequired,iprovided,ierror)
             ...
             if(iprovided .lt. irequired) print *, &
     &       "Warning: MPI not fully threadsafe! Provided is:",iprovided
!
...
!$OMP&         irkz,icount,mythread,ny_tloc,nz_tloc, &
!$OMP&          k_glob )
!
!$OMP MASTER
        num_threads = OMP_GET_NUM_THREADS()
!       num_threads = num_threads + 1        ! full number of threads!
!$OMP END MASTER
!$OMP BARRIER
!
        mythread = OMP_GET_THREAD_NUM()      ! From 0 to num_threads-1
!
        ny_tloc = ny_loc/num_threads   ! No. Y-points per thread (for this MPI process)
        nz_tloc = nz_loc/num_threads   ! No. Z-points per thread (for this MPI process)
!
!=====================================================
...
!debug      print *,'Okay to 4, icount,thread =', icount,mythread
!
!======================================================================
