...
void call_flannC(int id, int from, int to, int from_value, int to_value) {
	...
	float *temp;
		...
		//cout<<"Actual Cores"<<flann_threads;
		p.cores = flann_threads;
		cout<<"Cores  "<<p.cores<<"\n";
	for (int i = from; i < to; i++) {
		...
		index_id = flann_build_index(hdf5_data_files[i], keys_data[i], DIM,
				&speedup, &p);
		...
		flann_find_nearest_neighbors_index(index_id,&hdf5_query_files[MAX_IMAGES * client_packet.img_id], client_packet.num_keys,result, dists, NN, &p);
		...
		//sprintf(result_file, "/home/sanjana/Desktop/IS/results%d.dat",
		//pthread_self());
 ...
 * 	Function: dispatch_to_worker_threads
	...
	Returns: void
 */
void dispatch_to_worker_threads() {
