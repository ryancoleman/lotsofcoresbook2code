...
__declspec(target(mic)) typedef struct img_data {
	int num;
	int match_found;
} im_data;

//struct to hold data to be passed to a thread
__declspec(target(mic)) typedef struct str_thdata {
	int threadcount;
	int image_count;
	int from;
	int to;
...
void init_server() {
	...
	for (i = 0; i < MAX_IMAGES; i++) {
		...
		hdf5_data_files = (float *) malloc(MAX_IMAGES * MAX_IMAGES);
...
inline void generateKey_mmap(int count, int runsift) {
	...
	for (i = 0; i < count; i++) {
		...
		memcpy(&hdf5_data_files[MAX_IMAGES * i], tmp, file_size);
	}
 ...
 * 	Function: call_flannC
	...
	Returns: void
 */
__declspec(target(mic)) void call_flannC(int id, int from, int to, int from_value, int to_value) {
	...
	for (int i = from; i < to; i++) {
		...
		//index_id = flann_build_index(&hdf5_data_files[MAX_IMAGES * i], keys_data[i], DIM,
		//		&speedup, &p);
		index_id = flann_build_index(&hdf5_data_files[MAX_IMAGES * i], keys_data[i], DIM, &speedup, &p);
...
void send_results_to_client(int sock) {
	...
	if (image_matches == 0) {
		printf("\nFound no matches in server database printed from server\n");
		write(sock, &converted_number, sizeof(converted_number));
		return;
	}

	else {
		printf("\nSending top %d matches in server database printed from server\n", image_matches);
		write(sock, &converted_number, sizeof(converted_number));
	}
 ...
 * 	Function: dispatch_to_worker_threads
	...
	Returns: void
 */
__declspec(target(mic)) void dispatch_to_worker_threads(int num_data_images, int thread_count, thdata  *th_data, im_data *imageInfo) {
...
void dostuff(int sock) 
{
	...
	for (int k = 0; k < converted_int; k++) {
		...
		//memcpy(query_file, hdf5_query_files + k*file_size, file_size);
		double el_search;	
		double sum_offload = 0.0;
		timeval offloadstart;
		timeval offloadend;
		//gettimeofday(&offloadstart, NULL);
		// #pragma offload target(mic) inout(imageInfo) in(client_packet) in(hdf5_query_files:length(MAX_IMAGES*MAX_IMAGES) ALLOC) in(hdf5_data_files:length(MAX_IMAGES*MAX_IMAGES) ALLOC) in(keys_data) in(th_data:length(MAX_CORES) ALLOC)
		#pragma offload target(mic) inout(imageInfo) in(client_packet) in(hdf5_data_files:length(MAX_IMAGES*MAX_IMAGES) ALLOC) in(keys_data) in(th_data:length(MAX_CORES) ALLOC) in(query_file)
		{}

		#pragma offload target(mic) nocopy(imageInfo) nocopy(client_packet) nocopy(hdf5_data_files:length(MAX_IMAGES*MAX_IMAGES) BLAH) nocopy(keys_data) nocopy(th_data:length(MAX_CORES) BLAH) nocopy(query_file)
{
		gettimeofday(&offloadstart, NULL);
		dispatch_to_worker_threads(num_data_images, thread_count, th_data, imageInfo);
		gettimeofday(&offloadend, NULL);
}
		#pragma offload target(mic) out(imageInfo) out(num_data_images) nocopy(client_packet) nocopy(hdf5_data_files:length(MAX_IMAGES*MAX_IMAGES) FREE) nocopy(keys_data) nocopy(th_data:length(MAX_CORES) FREE) nocopy(query_file)
		{}
	
		//gettimeofday(&offloadend, NULL);
		el_search = (offloadend.tv_sec - offloadstart.tv_sec) * 1000.0; // sec to ms
		el_search += (offloadend.tv_usec - offloadstart.tv_usec) / 1000.0; // us to ms
		sum_offload += el_search;
		printf("Total time for offload: %g ms\n", sum_offload);
