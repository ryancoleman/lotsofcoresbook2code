...
//    int offload_target = nt_mic_local_device();
    int offload_target = get_local_mic();

    if (offload_target < 0)
    {
        perror("Error - can't find local mic card\n");
    ...
    // do part of calculation asynchronously on MIC
#if defined(MANUAL_NESTED)
    //workshare_offload = 0.71378;
    workshare_offload = 1.0;
#else
    workshare_offload=1.0;
#endif
    int sig=1;
    ...
    double toffload;
    double *gamma_flat_offload = (double *) malloc(gamma_total*sizeof(double));
#if defined(MANUAL_NESTED) || defined(OMP_NESTED)
    #pragma offload_transfer target(mic:offload_target) in(gamma_flat_offload[0:gamma_total] : ALLOC RETAIN)
    #pragma offload target(mic:offload_target) in(rank, numranks, gamma_size, workshare_offload) \
    out(gamma_flat_offload[0:gamma_total] : REUSE FREE) \
    out(toffload) \
    signal(sig)
    {
        double t1 = omp_get_wtime();
        #if defined(MANUAL_NESTED)
        gamma_3d_offload(gamma_flat_offload, gamma_size, rank, numranks, workshare_offload);
        #elif defined(OMP_NESTED)
        gamma_3d_offload_nested(gamma_flat_offload, gamma_size, rank, numranks, workshare_offload);
        #endif
        toffload = omp_get_wtime() - t1;
    }
#elif defined(OMP_TEAMS)
    {
        double t1 = omp_get_wtime();
        gamma_3d_offload_teams(gamma_flat_offload, gamma_size, rank, numranks, workshare_offload);
        toffload = omp_get_wtime() - t1;
    }
#endif
    ...
    // do other part of calculation on HOST
    //printf("host\n");
    //double t2 = omp_get_wtime();
    //gamma_3d_host(gamma_flat, gamma_size, rank, numranks, workshare_offload);
    //double thost = omp_get_wtime() - t2;

    printf("wait.\n");
    #ifdef __INTEL_OFFLOAD
#if defined(MANUAL_NESTED) || defined(OMP_NESTED)
    #pragma offload_wait target(mic:offload_target) wait(sig)
#endif
    //printf("[%d] Host main time = %f\n", rank, thost);
    printf("[%d] Offload main time = %f\n", rank, toffload);
    //printf("[%d] Diff = %f\n", rank, thost-toffload);
    for (i=0; i<gamma_total; i++)
        gamma_flat[i] += gamma_flat_offload[i];
    free(gamma_flat_offload);
    #endif
    ...
    if(rank==0)
    {
        puts("Writing gamma output to ./Output/part-2.output...");
        FILE *foutput = fopen("./Output/part-2.output", "w");
