...
int create_basis_late(int size, double max, double *vec){
	...
	if(eflag_order_late<4){
		...
		double **basis_temp = (double **)create_array(size,pmax_late-1);
	 	basis_functions_bi(basis_temp[0], size, pmax_late-2, max, vec);
...
// SJP: Swapped the order to be [n][l][i] for the 3D case
int read_beta(){
	...
	int n,l,i;
	double (*restrict beta)[pmax_prim+1][xsize_pad] = (double (*restrict)[pmax_prim+1][xsize_pad]) beta_flat;
