...
int main( int argc, char *argv[] ){
    ...
    // Main calculation. Done on the host in this version.
    if (rank==0)
        printf("Running on host\n");
    double t2 = omp_get_wtime();
    gamma_3d_host(gamma_flat, gamma_size, lmax);
    ...
    if(rank==0)
    {
        puts("Writing gamma output to ./Output/rmpi_V2.output...");
        FILE *foutput = fopen("./Output/rmpi_V2.output", "w");
